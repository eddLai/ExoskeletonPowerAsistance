...
async def read_specific_data_from_websocket(uri,ft_parameter):
    try:
...
async def process_data_from_websocket(data,ft_parameter):
    ...
    try:
        ...
        if "contents" in data_dict:
            ...
            try:
                ...
                for k in range(6):
                    emg_array[k], ft_parameter[k] = await process_emg_signal(emg_values[k],ft_parameter[k])
                return emg_array, ft_parameter
            except Exception as e:
                print(f"处理信号时发生错误: {e}")

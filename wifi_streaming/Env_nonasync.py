...
class ExoskeletonEnv(gym.Env):
    ...
    def async_step(self, action):
        ...
        new_observation, new_emg_observation, new_bp_parameter, new_nt_parameter, new_lp_parameter = client_order.get_INFO(self.reader, self.uri ,self.bp_parameter, self.nt_parameter, self.lp_parameter)
        ...
        client_order.send_action_to_exoskeleton(self.writer, action, self.observation ,"speed")
    ...
    def async_reset(self):
        ...
        self.reader, self.writer = client_order.connect_FREEX(self.host, self.port)
        client_order.FREEX_CMD(self.writer, "E", "0", "E", "0")
        self.observation, self.emg_observation, self.bp_parameter, self.nt_parameter, self.lp_parameter = client_order.get_INFO(self.reader, self.uri ,self.bp_parameter, self.nt_parameter, self.lp_parameter)
    ...
    def calculate_reward(self):
        reward, self.initial_max_min_rms_values = emgdata.calculate_emg_level(self.emg_observation, self.initial_max_min_rms_values, self.init_time)
        return reward
